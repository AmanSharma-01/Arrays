...
// Input
// Your program will be tested on a number of test cases. Each test case will be speciﬁed 
// on two separate lines. Each line denotes a sequence and is speciﬁed using the following format:
...
int main()
{
    ...
    while(i<n && j<m)
    {
        if(a1[i]<a2[j]) s1+=a1[i++];
        else if(a1[i]>a2[j]) s2+= a2[j++];
        else
        {
            ans += max(s1,s2) + a1[i];
            s1=s2=0;
